...
const useTypingEffect = (roles: string[], reducedMotion: boolean): string => {
...
export const Home = React.memo<React.FC>(() => {
	...
	const blinkStyle = useMemo(
		(): React.ReactNode => <style>{`@keyframes blink{50%{opacity:0}}`}</style>,
		[],
	);
	...
	return (
		<section
			id="home"
			className="min-h-screen flex items-center justify-center relative overflow-hidden py-24 md:py-0 select-none"
		>
			{blinkStyle}
			<BackGround />
						...
						<div className="flex-1 space-y-8 max-w-2xl mx-auto lg:mx-0">
							<div className="space-y-4">
								<div className="space-y-3">
									<h1
										className={`text-6xl sm:text-7xl lg:text-8xl font-black leading-tight tracking-tight will-change-transform ${reducedMotion
												? "transition-none"
									...
									<div className="flex items-baseline justify-left lg:justify-start gap-2">
										<span className="text-lg sm:text-2xl text-gray-300 font-light leading-none select-none">
											Hello, I am
										</span>
										...
										<span id="typing-description" className="sr-only">
											Typing animation showing different roles: Content Creator,
											Developer, Gamer
										</span>
									</div>
								</div>
								...
								<p className="text-gray-400 text-lg base:text-base leading-relaxed font-light select-none">
									Creating entertaining content, playing games to dominate the
									battleground, and building clean and responsive apps and great
									games.
								</p>
							</div>
