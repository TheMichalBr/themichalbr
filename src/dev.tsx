...
import { Canvas, useFrame, useThree } from "@react-three/fiber";
import { Color, Vector2 } from "three"; // Přidán Vector2
import { forwardRef, useRef, useMemo, useLayoutEffect, useState, useEffect } from "react";
...
uniform float uSpeed;
uniform vec2 uResolution; // Přidáno rozlišení

void main() {
    // Oprava poměru stran, aby se vlny nenatahovaly
    vec2 uv = vUv;
    uv.x *= uResolution.x / uResolution.y;
    
    float t = uTime * uSpeed;

    // Iterativní turbulence pro efekt husté tekutiny
    vec2 p = uv * 2.5; // Měřítko přiblížení
    ...
    // Organické mixování na základě nové deformace
    float mix1 = sin(p.x + p.y) * 0.5 + 0.5;
    float mix2 = sin(p.x * 0.5 - p.y * 0.5 + t * 0.5) * 0.5 + 0.5;

    // Plynulé prolnutí tří barev
    vec3 finalColor = mix(uColor1, uColor2, mix1);
    finalColor = mix(finalColor, uColor3, mix2);

    // Jemná viněta (ztmavení u okrajů pro lepší hloubku)
    vec2 center = vUv - 0.5; 
    float vignette = length(center);
    finalColor -= vignette * 0.35; 

    // Filmový šum proti pruhování barev
...
const LiquidPlane = forwardRef(function LiquidPlane({ uniforms }, ref) {
    const { viewport, size } = useThree(); // Přidán size pro rozlišení

    useLayoutEffect(() => {
        if (ref.current) {
            // Roztáhne plane přes celý viewport
            ref.current.scale.set(viewport.width, viewport.height, 1);
            
            // Aktualizuje rozlišení v shaderu při změně okna
            if (ref.current.material.uniforms.uResolution) {
                ref.current.material.uniforms.uResolution.value.set(size.width, size.height);
            }
        }
    }, [ref, viewport, size]);
...
LiquidPlane.displayName = "LiquidPlane";
...
const HomeBackGround = ({
    speed = 0.6, // Mírně zpomaleno pro tmavou eleganci
    color1 = "#000000", 
    color2 = "#0F172A", 
    color3 = "#00215E", 
}) => {
    ...
    const uniforms = useMemo(
        () => ({
            ...
            uTime: { value: 0 },
            uResolution: { value: new Vector2(1, 1) }, // Přidáno do uniformů
        }),
    ...
    const glConfig = useMemo(() => {
        if (deviceLevel === "low") {
            return { antialias: false, alpha: true, powerPreference: "low-power", precision: "lowp" };
        }
        if (deviceLevel === "mid") {
            return { antialias: false, alpha: true, powerPreference: "high-performance", precision: "mediump" };
        }
        return { antialias: true, alpha: true, powerPreference: "high-performance", precision: "highp" };
    }, [deviceLevel]);
    ...
    return (
        ...
        >
            <LiquidPlane ref={meshRef} uniforms={uniforms} />
        </Canvas>
    );
};
...
// Shader pro generativní terén chytající světlo
const vertexShader = `
...
// Klasický Perlin Noise (zjednodušený pro výkon)
...
void main() {
    ...
    // Vytvoření organického šumu na základě pozice a času
    float noise = snoise(uv * uNoiseDetail + t);
    
    // Aplikace výšky pouze na osu Z (zvedání terénu)
    vec3 newPosition = position;
    newPosition.z += noise * uElevation;

    // Předání dat do fragment shaderu pro stínování
    vNoise = noise;
    vNormal = normalize(normalMatrix * normal); // Aproximace normály pro stín
    
    vec4 mvPosition = modelViewMatrix * vec4(newPosition, 1.0);
    vViewZ = -mvPosition.z; // Hloubka pro mlhu
...
uniform vec3 uColorTop;    // Barva vrcholů vln (např. tmavě modrá)
uniform vec3 uColorBase;   // Barva základny (černá)
uniform float uFogNear;
uniform float uFogFar;

void main() {
    // Falešné světlo shora (čím víc normála míří nahoru, tím víc světla)
    float light = dot(vNormal, normalize(vec3(0.0, 1.0, 1.0))) * 0.5 + 0.5;
    light = pow(light, 3.0); // Zvýšení kontrastu stínů

    // Mix barev na základě výšky šumu (vrcholy vs údolí)
    float colorMix = smoothstep(-0.5, 0.8, vNoise);
    vec3 finalColor = mix(uColorBase, uColorTop, colorMix);

    // Aplikace světla na barvu
    finalColor *= light;

    // Hloubková mlha (do ztracena v dálce)
    float fogFactor = smoothstep(uFogNear, uFogFar, vViewZ);
    finalColor = mix(finalColor, uColorBase, fogFactor); // Mlha má barvu pozadí

    // Velmi jemné zrno (grain)
    float grain = fract(sin(dot(gl_FragCoord.xy, vec2(12.9898, 78.233))) * 43758.5453) * 0.02;

    gl_FragColor = vec4(finalColor - grain, 1.0);
}
`;

const DataLandscape = ({ deviceLevel }) => {
    const meshRef = useRef();
    const { size } = useThree();

    // Nastavení geometrie podle výkonu zařízení
    const segments = useMemo(() => {
        if (deviceLevel === "low") return 64;
        if (deviceLevel === "mid") return 128;
        return 256; // High-end PC zvládnou hodně polygonů
    }, [deviceLevel]);

    const uniforms = useMemo(() => ({
        uTime: { value: 0 },
        uSpeed: { value: 0.15 },            // Pomalý, majestátní pohyb
        uElevation: { value: 0.7 },        // Výška vln
        uNoiseDetail: { value: 3.0 },      // Hustota vln
        uColorTop: { value: new THREE.Color("#00215E") }, // Tvá tmavě modrá na špičkách
        uColorBase: { value: new THREE.Color("#523adf") }, // Černý základ
        uFogNear: { value: 2.0 },
        uFogFar: { value: 7.0 },
    }), []);
    ...
    useFrame((state, delta) => {
        ...
        // Jemná rotace celé scény pro paralaxní efekt
        meshRef.current.rotation.z = state.mouse.x * 0.05;
        meshRef.current.rotation.x = -Math.PI * 0.3 + (state.mouse.y * 0.05);
    });

    return (
        <mesh 
            ref={meshRef} 
            rotation={[-Math.PI * 0.3, 0, 0]} // Naklonění k divákovi
            position={[0, -1, 0]} // Posun mírně dolů
        >
            {/* PlaneGeometry s hodně segmenty, aby se dala deformovat */}
            <planeGeometry args={[15, 10, segments, segments]} />
            <shaderMaterial
                uniforms={uniforms}
                vertexShader={vertexShader}
                fragmentShader={fragmentShader}
                wireframe={false} // Změň na true, pokud chceš vidět síť ("Tron" styl)
                transparent={false}
            />
        </mesh>
    );
...
// Pomocné funkce pro detekci zařízení (stejné jako u tebe)
const getDeviceLevel = () => {
    if (typeof window === "undefined") return "high";
    const dpr = window.devicePixelRatio || 1;
    const memory = navigator.deviceMemory || 8;
...
const HomeBackGround = () => {
    ...
    const glConfig = useMemo(() => {
        // Základní konfigurace pro tmavou scénu
        const config = {
            antialias: deviceLevel === "high",
            alpha: false, // Černé pozadí
            powerPreference: "high-performance",
        };
        if (deviceLevel === "low") config.precision = "lowp";
        else if (deviceLevel === "mid") config.precision = "mediump";
        else config.precision = "highp";
    ...
    return (
        <Canvas
            ...
            frameloop="always"
            gl={glConfig}
            camera={{ position: [0, 0, 4], fov: 50 }} // Nastavení kamery
            style={{
                position: "absolute",
                top: 0,
                left: 0,
                ...
                zIndex: 0,
                pointerEvents: "none",
                background: "#000000", // Černý podklad
            }}
        >
            <DataLandscape deviceLevel={deviceLevel} />
        </Canvas>
    );
};
...
void main() {
    ...
    // Tekutá deformace UV souřadnic
    vec2 distortedUv = uv;
    distortedUv.x += sin(uv.y * 4.0 + t) * 0.15;
    distortedUv.y += cos(uv.x * 4.0 + t) * 0.15;

    // Vícevrstvé vlny pro organický pohyb
    float wave1 = sin(distortedUv.x * 5.0 + t) * cos(distortedUv.y * 5.0 + t * 0.8);
    float wave2 = sin(distortedUv.x * 3.0 - t * 0.5) * sin(distortedUv.y * 7.0 - t * 0.6);

    // Převod vln do rozmezí 0.0 -> 1.0 pro mixování barev
    ...
    // Jemný filmový šum (grain) proti "bandingu" (pruhování) barev na displeji
    float grain = fract(sin(dot(gl_FragCoord.xy, vec2(12.9898, 78.233))) * 43758.5453) * 0.04;
...
const EquipmentCard = ({ item, index, onClick }) => {
  ...
  const displaySpecs = useMemo(
    () => Object.entries(item.specs).slice(0, 1),
    [item.specs]
  );

  return (
    <article
      className={`group relative bg-linear-to-br from-[#1e20243a] to-[#2a2d353a] border border-white/10 rounded-xl overflow-hidden shadow-md cursor-pointer transition-all duration-300 hover:shadow-lg hover:border-blue-400/40 hover:bg-linear-to-br hover:from-slate-800/60 hover:to-slate-900/60 focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:ring-offset-2 focus:ring-offset-slate-900 flex items-center gap-4 p-4 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-8"
        }`}
      onClick={handleClick}
      role="button"
      tabIndex={0}
    ...
    >
      {/* Left: Image/Icon */}
      <div className="relative w-16 h-16 flex-shrink-0 bg-linear-to-br from-white/5 to-transparent rounded-lg flex items-center justify-center overflow-hidden">
        {!imageLoaded && (
      ...
      <div className="flex-1 min-w-0">
        ...
        <h3 className="text-sm font-semibold text-gray-200 group-hover:text-blue-100 transition-colors duration-300 line-clamp-2">
          {item.name}
        </h3>
      </div>
      ...
      <div className="flex-shrink-0 text-right">
        {displaySpecs.length > 0 && (
          <dl className="text-xs text-gray-400">
            {displaySpecs.map(([key, value]) => (
              <div key={key}>
                <dt className="capitalize font-medium text-gray-400 text-xs">
                  {formatSpecKey(key)}
                </dt>
                <dd className="text-gray-200 font-semibold truncate">
                  {value}
                </dd>
              </div>
            ))}
      ...
      <div className="absolute inset-0 bg-linear-to-r from-blue-500/5 via-transparent to-purple-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
    </article>
  );
};
    ...
    <section id="equipment" className="min-h-screen py-20 lg:px-10">
      <div className="max-w-6xl mx-auto px-4 select-none">
        <RevealOnScroll>
          ...
          <div
            className={`flex flex-col gap-3 transition-opacity duration-300 ${isFilterChanging ? "opacity-0" : "opacity-100"
              }`}
          >
...
const getCategoryColor = (categoryGroup) => {
  const colors = {
    COMPUTER: { bg: "from-orange-500/10 to-orange-600/5", border: "border-orange-400/20", text: "text-orange-300", label: "text-orange-200" },
    PERIPHERALS: { bg: "from-green-500/10 to-green-600/5", border: "border-green-400/20", text: "text-green-300", label: "text-green-200" },
    MONITORS: { bg: "from-purple-500/10 to-purple-600/5", border: "border-purple-400/20", text: "text-purple-300", label: "text-purple-200" },
    AUDIO: { bg: "from-yellow-500/10 to-yellow-600/5", border: "border-yellow-400/20", text: "text-yellow-300", label: "text-yellow-200" },
    "MOBILE & CONSOLES": { bg: "from-pink-500/10 to-pink-600/5", border: "border-pink-400/20", text: "text-pink-300", label: "text-pink-200" },
    OTHER: { bg: "from-cyan-500/10 to-cyan-600/5", border: "border-cyan-400/20", text: "text-cyan-300", label: "text-cyan-200" },
  };
  return colors[categoryGroup] || colors.OTHER;
};

const getCategoryGroupForItem = (category) => {
  const group = getCategoryGroup(category);
  return group === "COMPUTER" && (category.includes("MONITOR") || category.includes("SIDE")) ? "MONITORS" : group;
};
...
const EquipmentCard = ({ item, index, onClick }) => {
  ...
  const categoryGroup = getCategoryGroupForItem(item.category);
  const categoryColors = getCategoryColor(categoryGroup);

  return (
    <article
      className={`group relative bg-linear-to-br from-[#1e20243a] to-[#2a2d353a] border border-white/10 rounded-xl overflow-hidden shadow-md hover:shadow-2xl cursor-pointer transition-all duration-300 hover:border-blue-400/50 hover:bg-linear-to-br hover:from-slate-800/60 hover:to-slate-900/60 focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:ring-offset-2 focus:ring-offset-slate-900 flex items-center gap-3 md:gap-5 p-3 md:p-4 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-8"
        } before:absolute before:inset-0 before:bg-white/5 before:opacity-0 group-hover:before:opacity-100 before:transition-opacity before:duration-500 before:rounded-xl before:pointer-events-none`}
      onClick={handleClick}
      role="button"
      tabIndex={0}
      onKeyDown={handleKeyDown}
    ...
    >
      {/* Left: Image/Icon */}
      <div className="relative w-16 h-16 md:w-24 md:h-24 flex-shrink-0 bg-linear-to-br from-white/5 to-transparent rounded-lg flex items-center justify-center overflow-hidden group-hover:shadow-lg group-hover:shadow-blue-500/20 transition-shadow duration-300">
        {!imageLoaded && (
          <div className="absolute inset-0 bg-gradient-to-r from-gray-800 via-gray-700 to-gray-800 animate-pulse" />
        )}

        {imageError ? (
          <div className="flex items-center justify-center text-gray-500 w-full h-full bg-gray-900/50">
            <svg
          ...
          <img
            src={item.image}
            alt={`${item.name} - ${item.category}`}
            className={`w-full h-full object-contain transition-all duration-400 group-hover:scale-125 ${imageLoaded ? "opacity-100" : "opacity-0"
              } relative z-10`}
            onLoad={handleImageLoad}
            onError={handleImageError}
            loading="lazy"
        ...
        <div className={`absolute top-1 right-1 w-4 h-4 md:w-5 md:h-5 bg-gradient-to-br ${categoryColors.bg} border ${categoryColors.border} rounded-full flex items-center justify-center text-xs font-bold ${categoryColors.text} opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20`}>
          {categoryGroup.charAt(0)}
        </div>
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="mb-0.5">
          <span className={`text-xs font-bold uppercase tracking-widest opacity-70 group-hover:opacity-100 transition-all duration-300 ${categoryColors.label}`}>
            {item.category}
          </span>
        </div>
        <h3 className="text-sm md:text-base font-semibold text-gray-100 group-hover:text-blue-100 transition-colors duration-300 line-clamp-2 relative z-10">
          {item.name}
        </h3>
      </div>

      {/* Right: Specs - Horizontal Layout */}
        ...
        {displaySpecs.length > 0 && (
          <div className="flex gap-2">
            {displaySpecs.map(([key, value]) => (
              <div
                key={key}
                className={`bg-gradient-to-br ${categoryColors.bg} border ${categoryColors.border} rounded-lg px-2.5 py-2 backdrop-blur-sm transition-all duration-300 hover:scale-105 origin-center transform`}
              >
                <dt className={`text-xs font-medium ${categoryColors.text} uppercase tracking-wide mb-0.5 transition-colors duration-300`}>
                  {formatSpecKey(key)}
                </dt>
                <dd className="text-white text-xs font-semibold break-words max-w-[110px] line-clamp-2">
                  {value}
                </dd>
              </div>
            ))}
      ...
      <div className="flex-shrink-0 ml-auto relative z-10">
        <svg
          className="w-5 h-5 text-gray-400 group-hover:text-blue-400 transition-all duration-300 group-hover:translate-x-1"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      {/* Glow effect */}
      <div className="absolute inset-0 bg-linear-to-r from-blue-500/5 via-transparent to-purple-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-xl" />
      <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-600/20 to-purple-600/20 rounded-xl opacity-0 group-hover:opacity-50 blur-xl transition-opacity duration-500 pointer-events-none -z-10" />
    </article>
  );
};
    ...
    <section id="equipment" className="min-h-screen py-20 lg:px-10">
      <div className="max-w-6xl mx-auto px-4 select-none">
        <RevealOnScroll>
          ...
          >
            {filteredEquipment.map((item, index) => (
              <div key={`${item.name}-${selectedFilter}-${searchTerm}`}>
                <EquipmentCard
                  item={item}
                  index={index}
                  onClick={handleItemSelect}
                ...
                {index < filteredEquipment.length - 1 && (
                  <div className="h-px bg-gradient-to-r from-transparent via-blue-500/20 to-transparent mt-3" />
                )}
              </div>
            ))}
          </div>
...
const EquipmentCard = ({ item, index, onClick }) => {
  ...
  return (
    <article
      className={`group relative bg-gradient-to-br from-[#1e20243a]/80 to-[#2a2d353a]/80 border border-white/10 rounded-xl overflow-hidden shadow-md hover:shadow-2xl cursor-pointer transition-all duration-300 hover:border-blue-400/50 backdrop-blur-md hover:backdrop-blur-lg focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:ring-offset-2 focus:ring-offset-slate-900 flex items-center gap-3 md:gap-5 p-3 md:p-4 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-8"
        } before:absolute before:inset-0 before:bg-white/5 before:opacity-0 group-hover:before:opacity-100 before:transition-opacity before:duration-500 before:rounded-xl before:pointer-events-none`}
      onClick={handleClick}
    ...
    >
      ...
      <div className={`absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b ${categoryColors.bg} opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />
      {/* Left: Image/Icon */}
      <div className="relative w-16 h-16 md:w-24 md:h-24 flex-shrink-0 bg-linear-to-br from-white/5 to-transparent rounded-lg flex items-center justify-center overflow-hidden group-hover:shadow-lg group-hover:shadow-blue-500/20 transition-all duration-300">
        {!imageLoaded && (
          <div className="absolute inset-0 bg-gradient-to-r from-gray-800 via-gray-700 to-gray-800 animate-pulse" />
        )}

        {/* Shimmer effect */}
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" style={{background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.2), transparent)', animation: 'shimmer 2s infinite'}} />
        ...
        {imageError ? (
          ...
          <img
            src={item.image}
            alt={`${item.name} - ${item.category}`}
            className={`w-full h-full object-contain transition-all duration-500 group-hover:brightness-110 filter ${imageLoaded ? "opacity-100" : "opacity-0"
              } relative z-10`}
            onLoad={handleImageLoad}
            onError={handleImageError}
            loading="lazy"
      ...
      <div className="flex-1 min-w-0">
        ...
        <h3 className="text-sm md:text-base font-semibold text-gray-100 group-hover:text-blue-100 transition-colors duration-300 line-clamp-2 relative z-10 leading-snug">
          {item.name}
        </h3>
      </div>
      ...
      {/* Right Arrow - Chevron */}
      <div className="flex-shrink-0 ml-auto relative z-10 flex items-center gap-2">
        {/* Tooltip */}
        <div className="absolute -bottom-8 right-0 bg-gray-900 text-white text-xs px-2 py-1 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none shadow-lg border border-gray-700">
          Klikni pro detaily
        </div>
        <svg
          className="w-5 h-5 text-gray-400 group-hover:text-blue-400 transition-all duration-300 group-hover:translate-x-1.5"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
